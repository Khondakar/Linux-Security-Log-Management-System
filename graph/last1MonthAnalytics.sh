...
emergencyCount=$(sudo journalctl --since "1 month ago" -p 0 | wc -l)
alertCount=$(sudo journalctl --since "1 month ago" -p 1 | wc -l)
criticalCount=$(sudo journalctl --since "1 month ago" -p 2 | wc -l)
errorCount=$(sudo journalctl --since "1 month ago" -p 3 | wc -l)
warningCount=$(sudo journalctl --since "1 month ago" -p 4 | wc -l)
noticeCount=$(sudo journalctl --since "1 month ago" -p 5 | wc -l)

# Display data with counts:
echo -e "\e[96m## All Important Reports - from last 1 month ## "
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
echo "1 Emergency $emergencyCount" > ./graph/monthlydata.txt
echo "2 Alert $alertCount" >> ./graph/monthlydata.txt
echo "3 Critical $criticalCount" >> ./graph/monthlydata.txt
echo "4 Error $errorCount" >> ./graph/monthlydata.txt
echo "5 Warning $warningCount" >> ./graph/monthlydata.txt
echo "6 Notice $noticeCount" >> ./graph/monthlydata.txt

# Run the gnuplot using the setting of plotting a graphy
gnuplot ./graph/graphPlotingMonthly.plt

# Display the graph saved as PNG file generate with data
eog CriticalReportMonthly.png
